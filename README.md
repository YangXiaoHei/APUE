...
> 运行书上的代码如下：无法成功复制文件，总是报错，显示 `aio_suspend error : Invalid Argument `
...
> 先检测当前的上一次访问时间：
 ...
 ~~~C
./atime ~/Desktop/src.mp4 
access time :  1532783058157888731
 ~~~
 
> 复制：

~~~C
./copy ~/Desktop/src.mp4 dst.mp4
~~~

> 检测当前的上一次访问时间：

~~~C
access time :  1532783365841656948
~~~
> 可以发现，上一次访问时间已经被更新
...
> 可能会破坏客户进程和服务器进程通信的协议格式；只要知道消息队列的标识符就可以读消息队列。
