...
int main(int argc, char *argv[]) {
    ...
    if ((pid = fork()) < 0) {
        ...
        sleep(1);
        printf("first child begin to obtain read lock\n");
        read_lock(fd);
        printf("first child hold read lock\n");
        sleep(2);
        unlock(fd);
        printf("first child release read lock\n");
        exit(0);
    }
    ...
    if ((pid = fork()) < 0) {
        ...
        sleep(3);
        printf("second child begin to obtain write lock\n");
        write_lock(fd);
        printf("second child hold write lock\n");
        sleep(4);
        unlock(fd);
        printf("second child release write lock\n");
        exit(0);
    }
